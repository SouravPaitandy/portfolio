...
//     try {
//       // Try a different counter service
//       const response = await axios.get(
//         "https://api.countapi.io/hit/sourav-portfolio/visits"
//       );
      
//       if (response.data && typeof response.data.value === "number") {
//         setVisitorCount(response.data.value);
//         localStorage.setItem("lastKnownVisitorCount", response.data.value.toString());
//       } else {
...
//       const lastCount = parseInt(localStorage.getItem("lastKnownVisitorCount") || "100", 10);
//       if (!sessionStorage.getItem("counted")) {
//         sessionStorage.setItem("counted", "true");
//         localStorage.setItem("lastKnownVisitorCount", (lastCount + 1).toString());
//         setVisitorCount(lastCount + 1);
//       } else {
//         setVisitorCount(lastCount);
//       }
//     } finally {
...
//   // Add this effect to cache the count when it changes
//   useEffect(() => {
//     if (visitorCount > 1) {
...
//   }, [visitorCount]);
  ...
  return (
    ...
    >
      ...
      {document.body.clientWidth > 640 && themeMode === "dark" && (
        <WebGLBackground />
      )}
      ...
      {/* <motion.div
        className="visitor-badge absolute bottom-8 right-8 md:bottom-12 md:right-12"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 1.8 }}
      >
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full text-sm bg-white/10 dark:bg-gray-800/30 backdrop-blur-md border border-cyan-500/20">
          ...
          <span className="font-mono text-gray-700 dark:text-gray-200">
            {isCountLoading ? (
              ...
              `${visitorCount.toLocaleString()} visitor${
                visitorCount !== 1 ? "s" : ""
              }`
            )}
          </span>
