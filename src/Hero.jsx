...
const AntigravityParticles = React.memo(() => {
  ...
  return <div ref={mountRef} className="absolute inset-0 z-0 pointer-events-none opacity-60 dark:opacity-40" />;
});
    ...
    >
      ...
      >
        ...
        <motion.path
          ...
          animate={{ stroke: isHovered ? `url(#${ariaLabel}Gradient)` : "none", strokeWidth: isHovered ? 1 : 0 }}
...
export default function Hero() {
  ...
  return (
    <motion.section className="hero-section relative min-h-screen flex flex-col justify-center items-center overflow-hidden bg-white dark:bg-rich-black text-gray-900 dark:text-white z-30 transition-colors duration-300">
      ...
      {/* Content */}
      <div className="relative flex flex-col justify-center items-center z-10 max-w-5xl px-4 md:px-6 text-center mt-[2vh]">
        ...
        >
          ...
          <span className="text-gray-900 dark:text-white font-medium"> precision</span>,
          <span className="text-gray-900 dark:text-white font-medium"> aesthetics</span>, and
          <span className="text-gray-900 dark:text-white font-medium"> scalability</span>.
        </motion.p>
        ...
        >
          <Link to="project-section" smooth={true} duration={500} className="w-full sm:w-auto">
            <motion.button
              ...
              className="px-8 py-3 md:px-8 md:py-4 rounded-full bg-electric-indigo text-white font-bold text-base md:text-lg hover:bg-indigo-600 transition-colors shadow-[0_0_20px_rgba(99,102,241,0.3)] w-full sm:w-auto"
            >
              View Projects
            </motion.button>
          </Link>
        ...
        >
          {socialLinks.map((link, index) => (
            <div key={index} onClick={() => trackEvent("Hero", "Social Click", link.icon)}>
                <AnimatedSocialIcon
                path={link.svgPath}
                color={link.color}
                href={link.href}
