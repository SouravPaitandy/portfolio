...
//
// Responsibilities:
//   • Declare all <head> metadata via Next.js Metadata API (replaces index.html)
//   • Load Google Fonts via next/font (replaces CDN <link> tags in index.html)
//   • Emit the JSON-LD structured-data <script> tag server-side
//   • Wrap children with <Providers> (the single client boundary for theme + i18n)
// ─────────────────────────────────────────────────────────────────────────────
...
// ── Metadata (replaces all <meta> tags in index.html) ───────────────────────
export async function generateMetadata({ params }) {
  const { lang } = params;
  const canonicalUrl = `https://souravpaitandy.dev/${lang}/`;
...
// ── JSON-LD Structured Data (verbatim from index.html) ──────────────────────
const jsonLd = {
...
export default function RootLayout({ children, params }) {
  return (
    <html
      lang={params.lang}
      // suppressHydrationWarning is required because <Providers> adds
      // a .dark / .light class to <html> on the client after mount.
      // Without it, React would warn about a server/client class mismatch.
      suppressHydrationWarning
    >
      <head>
        {/* theme-color is not yet in the Next.js Metadata API stable surface */}
        <meta name="theme-color" content="#115e59" />
        {/* FOUC prevention script for ThemeMode */}
        <script
        ...
        {/* JSON-LD emitted server-side — crawlers see this without JS */}
        <script
      ...
      >
        {/*
          Providers is the client boundary. Everything below it in the React
          tree has access to ThemeContext and the initialised i18n instance.
        */}
